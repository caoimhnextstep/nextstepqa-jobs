...
// Only includes verified working slugs
// ============================================================
...
const QA_TITLE_EXCLUDE = [
  ...
  "backend", "data engineer", "product manager", "product designer",
  "marketing", "sales", "finance", "people", "recruiter", "analyst",
  "devops", "mobile engineer", "ios", "android", "account manager",
  "social media", "technical writer", "legal", "operations",
  "react native", "web developer", "software engineer", "support engineer",
  "support specialist", "support agent", "growth", "ml science",
...
async function fetchGreenhouse(source) {
  ...
  return (data.jobs || []).filter(j => isQARole(j.title)).map(j => normaliseGreenhouse(j, source));
}
...
async function fetchLever(source) {
  ...
  const data = await res.json();
  return (Array.isArray(data) ? data : []).filter(j => isQARole(j.text)).map(j => normaliseLever(j, source));
}
